...
install the newest KindredCommands release together with the exact recursive
dependency versions declared by that release, and run the resulting server
under Wine.
...
The newest VCF release is newer than the version declared by KindredCommands.
The resolver must therefore select the newest KindredCommands root and preserve
the exact dependency versions in its manifest. It must never independently
...
- **package-lock.json** records the selected root, exact dependency closure,
...
1. Reads Thunderstore's package endpoint for the latest KindredCommands
   version, unless **KINDRED_COMMANDS_VERSION** specifies an exact version.
2. Traverses that exact version's dependency strings recursively.
...
VCF, KindredCommands, and Kindred's bundled NetTopologySuite library are placed
in one image-managed plugin subtree. Bloodstone and ServerLaunchFix are not
installed because they are not dedicated-server dependencies.
...
native-then-builtin. **MODS_ENABLED=false** selects Wine's builtin winhttp and
starts unmodded without deleting package caches, generations, or configuration.
...
| MODS_ENABLED | true | Launch with the managed BepInEx and Kindred stack. |
...
- Configuration defaults, strict parsing, alias precedence, and ownership
  selection.
- Latest-root selection, exact recursive resolution, duplicate requirements,
  cycles, conflicts, inactive versions, and malformed dependency strings.
...
- Independent dependency upgrades that override KindredCommands manifests.
